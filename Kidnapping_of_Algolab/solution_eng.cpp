 ...
 * @param [out] people: set of people, each person is represented by an integer from 1 to N
 */
void elimination(int bullet_count, int bullets[], long int people_count, std::set<int> &people) {
    int i = 0, j = 0; // i: bullet index, j: unused
    ...
    while (people.size() > 1) { // O(N)
        ...
        // Determines if the current bullet is used or not:
        // bullets[i] == 0 → not used 
        // bullets[i] == 1 → used 
        if (bullets[i] == 0) {
            bullets[i] = 1;
            ...
            // Save next iterator before erasing current person
